...
argument-hint: "[path to .issues/<issue-name>/spec.md] [target branch]"
...
2. **Spec**: the `.issues/<issue-name>/spec.md` the argument names; else, when exactly one spec folder in `.issues/` has open tickets, that one; else ask. The final review needs it.
3. **Graph**: read every open ticket of that spec (`.issues/<issue-name>/tickets/`, or `gh issue list --label issue:<issue-name>`) and its `blocked-by`. A cycle stops here and goes to the user.
...
2. PASS: delete `.issues/<issue-name>/` (local) or confirm every issue is closed and delete the spec folder (GitHub); delete the ticket branches.
